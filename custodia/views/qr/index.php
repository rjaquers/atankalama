<html lang='es'>
<head>
    <meta charset='UTF-8'>
    <title>Lector QR – CI Chile</title>
    <link href='https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css' rel='stylesheet'>

    <!-- Librería de lectura QR -->
    <script src='https://unpkg.com/html5-qrcode'></script>

    <style>
        #reader {
            width: 320px;
            margin: auto;
        }
    </style>
</head>
<body class='container py-4'>

<h3 class='text-center'>Escanear QR – Cédula de Identidad</h3>
<p class='text-center'>Apunte la cámara al QR del carnet. Se procesará automáticamente.</p>

<div id='reader'></div>

<div class='mt-4 text-center'>
    <div id='resultado' class='alert alert-info d-none'></div>
</div>

<script>
    function procesarQR(qrData) {

        let r = document.getElementById('resultado');
        r.classList.remove('d-none');
        r.innerHTML = 'QR capturado: <strong>' + qrData + '</strong><br>Enviando al servidor...';

        // ======================================================
        //  🔴 CAMBIO IMPORTANTE:
        //     Usar URL ABSOLUTA para evitar error JSON.parse
        // ======================================================
        fetch('https://www.atankalama.com/colaciones/qr/validar', {
            method: 'POST',
            body: JSON.stringify({ qr: qrData }),
            headers: { 'Content-Type': 'application/json' }
        })
            .then(res => res.json())
            .then(data => {

                if (data.estado !== 'ok') {
                    r.innerHTML += '<br><strong>Error PHP:</strong> ' + data.mensaje;
                    return;
                }

                let d = data.datos;

                r.innerHTML = `
                <h4>Datos extraídos del QR</h4>
                <table class="table table-bordered mt-3 text-start">
                    <tr><th>RUT</th><td>${d.rut}</td></tr>
                    <tr><th>Nombre</th><td>${d.nombre}</td></tr>
                    <tr><th>Tipo Documento</th><td>${d.tipo}</td></tr>
                    <tr><th>N° Serie</th><td>${d.serial}</td></tr>
                    <tr><th>MRZ</th><td>${d.mrz}</td></tr>
                </table>

                <hr>

                <small><strong>QR original:</strong><br>${data.qr_original}</small>
            `;
            })
            .catch(e => {
                r.innerHTML += '<br>Error: ' + e;
            });
    }

    // Inicializar cámara
    new Html5Qrcode('reader').start(
        { facingMode: 'environment' },
        {},
        procesarQR
    );
</script>

</body>
</html>
