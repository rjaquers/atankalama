...
<body class='pro-body'>
    ...
    <div class='container py-4'>
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 pb-2 border-bottom"
            ...
            <h2 class="mb-0 fw-bold">
                <i class="bi bi-building-fill me-2" style="color: var(--color-cta)"></i>Nueva Orden — Empresa / Organización
            </h2>
        ...
        <form method='POST' action='index.php?page=recepcion/crear' id='ordenForm' enctype='multipart/form-data'>
            <input type="hidden" name="tipo_solicitante" value="empresa">
            ...
            <div class="pro-card border-0 mb-4">
                <div class="card-header bg-transparent py-3 px-4" style="border-bottom: 1px solid var(--color-border);">
                    <h5 class="fw-bold mb-0" style="color: var(--color-primary);">
                        <i class="bi bi-building me-2" style="color: var(--color-cta)"></i>Datos de la Empresa
                    </h5>
                </div>
                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-md-6">
                            <label class="form-label text-muted fw-bold d-flex align-items-center">
                                <i class="bi bi-building fs-5 me-2 text-primary"></i>Empresa
                                <span class="text-danger ms-1">*</span>
                            </label>
                            <select class="form-select form-select-lg border-0 shadow-none" name="company_id"
                                id="selectEmpresa" required>
                                <option value="">— Selecciona una empresa —</option>
                                <?php foreach ($empresasLista as $emp): ?>
                                    <option value="<?= $emp['id'] ?>"
                                        data-contacto="<?= htmlspecialchars($emp['contact_name'] ?? '') ?>"
                                        data-email="<?= htmlspecialchars($emp['contact_email'] ?? '') ?>">
                                        <?= htmlspecialchars($emp['business_name']) ?>
                                        <?php if (!empty($emp['trade_name']) && $emp['trade_name'] !== $emp['business_name']): ?>
                                            — <?= htmlspecialchars($emp['trade_name']) ?>
                                        <?php endif; ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <!-- Contrato (cargado vía AJAX) -->
                        <div class="col-md-6">
                            <label class="form-label text-muted fw-bold d-flex align-items-center">
                                <i class="bi bi-file-text fs-5 me-2 text-primary"></i>Contrato asociado
                                <span class="text-muted fw-normal small ms-2">(opcional)</span>
                            </label>
                            <select class="form-select form-select-lg border-0 shadow-none" name="contract_id"
                                id="selectContrato" disabled>
                                <option value="">— Sin contrato / Elige empresa primero —</option>
                            </select>
                            <div class="form-text text-muted">
                                <i class="bi bi-info-circle me-1"></i>Se carga automáticamente al elegir empresa.
                            </div>
                        </div>
                        ...
                        <div class="col-md-6">
                            <label class="form-label text-muted fw-bold d-flex align-items-center">
                                <i class="bi bi-person-vcard fs-5 me-2 text-primary"></i>Nombre del contacto / encargado
                                <span class="text-danger ms-1">*</span>
                            </label>
                            <input type="text" class="form-control form-control-lg border-0 shadow-none"
                                name="nombre_contacto" id="inputNombreContacto" required
                        ...
                        <!-- Archivo de respaldo -->
                        <div class="col-12">
                            <div class="p-3 rounded-4 border border-warning border-opacity-50"
                                style="background-color: var(--color-surface);">
                                <label class="form-label text-warning-emphasis fw-bold d-flex align-items-center mb-2">
                                    <i class="bi bi-paperclip fs-5 me-2"></i>Respaldo de la solicitud
                                    <span class="text-muted fw-normal small ms-2">(opcional pero recomendado)</span>
                                </label>
                                <input type="file" class="form-control border-0 shadow-none"
                                    name="archivo_respaldo" id="inputArchivoRespaldo"
                                    accept="image/*,.pdf">
                                <div class="form-text mt-2">
                                    <i class="bi bi-whatsapp text-success me-1"></i>
                                    Puedes adjuntar una captura del chat de WhatsApp, correo u otro documento que
                                    respalde la solicitud. Formatos aceptados: imágenes o PDF.
                                </div>
                            </div>
                        </div>
            ...
            <div class="pro-card border-0 mb-4">
                ...
                <div class="card-body px-4 pb-4 pt-4">
                    <div class="row g-3">
                        <?php foreach ($productosLista as $p): ?>
                            <div class="col-lg-3 col-md-4 col-sm-6">
                                <input type="checkbox" class="btn-check" name="productos[<?= $p['nombre'] ?>]"
                                    value="<?= $p['precio'] ?>" id="prod<?= str_replace(' ', '', $p['nombre']) ?>"
                                    autocomplete="off">
                                <label class="btn btn-outline-secondary w-100 text-start d-flex justify-content-between align-items-center py-2 px-3 shadow-none border-1"
                                    ...
                                    <span class="badge bg-primary rounded-pill fw-normal fs-6 px-3">
                    ...
                    <div class='row g-3 mt-3 p-4 rounded-4 border border-warning border-opacity-50' id="div_otros"
                        ...
                        <div class='col-md-8'>
                            <label class='form-label text-warning-emphasis fw-bold d-flex align-items-center'>
                                <i class="bi bi-card-text me-2"></i>Descripción detallada del otro producto
                            </label>
                        ...
                        <div class='col-md-7'>
                            <div class="bg-transparent p-3 rounded-4 border"
                                ...
                                <input type='text' class='form-control form-control-lg border-0 shadow-none'
                                    ...
                                    placeholder="Introduce el nro. comprobante (requerido si está pagado)"
            ...
            <div class="pro-card border-0 mb-5 pb-0 rounded-4 overflow-hidden">
                <div class="card-body p-0 d-flex justify-content-between align-items-stretch flex-wrap">
                    <div class="d-flex align-items-center p-4 bg-transparent flex-grow-1 border-end"
                        ...
                        <div>
                            <span class="text-muted text-uppercase fw-bold small d-block tracking-wide">Total Estimado Orden</span>
    ...
    <script>
        // ── Carga contratos vía AJAX al elegir empresa ───────────────────────
        document.getElementById('selectEmpresa').addEventListener('change', function () {
            ...
            const inputContacto  = document.getElementById('inputNombreContacto');
            const inputObs       = document.getElementById('inputObservaciones');
            const selectedOption = this.options[this.selectedIndex];

            if (companyId) {
                inputContacto.value = selectedOption.dataset.contacto || '';
                inputObs.value      = ''; // Limpiar observaciones al cambiar empresa
            } else {
                inputContacto.value = '';
                inputObs.value      = '';
            }

            selectContrato.innerHTML = '<option value="">Cargando...</option>';
            selectContrato.disabled  = true;

            if (!companyId) {
                selectContrato.innerHTML = '<option value="">— Sin contrato / Elige empresa primero —</option>';
                return;
            }

            fetch('index.php?page=recepcion/contratosEmpresa&company_id=' + companyId)
                .then(r => r.json())
                .then(contratos => {
                    selectContrato.innerHTML = '<option value="">— Sin contrato (opcional) —</option>';
                    if (contratos.length === 0) {
                        selectContrato.innerHTML += '<option value="" disabled>Esta empresa no tiene contratos activos</option>';
                    } else {
                        contratos.forEach(c => {
                            const estado = c.status.charAt(0).toUpperCase() + c.status.slice(1);
                            const label  = `${c.code} — ${c.contract_type} [${estado}]`;
                ...
                .catch(() => {
                    selectContrato.innerHTML = '<option value="">Error al cargar contratos</option>';
                    selectContrato.disabled  = false;
                });
        });
        ...
        function calcularTotal() {
            const personas = parseInt(document.getElementById('cantidad_personas').value) || 1;
            let subtotal = 0;
            document.querySelectorAll('input[name^="productos"]').forEach(el => {
                if (el.checked) subtotal += parseFloat(el.value);
            });
            ...
            if (otrosDesc && !isNaN(otrosPrecio)) subtotal += otrosPrecio;
            document.getElementById('total').value = (subtotal * personas).toLocaleString('es-CL');
        }
        document.querySelectorAll('input[name^="productos"], #otros_desc, #otros_precio, #cantidad_personas')
